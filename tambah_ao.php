        ...
        <script>
			$().ready(function(){
                $('#prov').submit(function(e){
                    ...
                    $.ajax({
                        'type': 'POST',
                        'url': 'addtable/help_desk_tambahx.php',
                        'data': $(this).serialize(),
						'cache': false,
						'async': true,
...
<header><h3 class="tabs_involved">Tambah AO</h3></header>
<div class="row">
...
<div class="col-lg-12">
                    <div class="bs-example">
                        <form id="prov" method="post">
							<input class="col-lg-8"  name="cbg" placeholder="masukan kode cbg" required></input>
							<input type="submit" value='panggil'/>
							<button type="reset" onClick="location.reload()">Reset</button>
						</form>
									<div id="tambah_ao"></div>
                    </div>
                </div>
				</article>
					</div>
