        ...
        <script>
			$().ready(function(){
                $('#Input3').submit(function(e){
                    e.preventDefault();
					$('#process6').html('<img  src="images/loading.gif"/>');
					$('#process6').slideDown('slow');
                    $.ajax({
                        'type': 'POST',
                        'url': 'addtable/help_desk_process6',
                        'data': $(this).serialize(),
						'cache': false,
						'async': true,
...
<div class="col-lg-12">
                    <div class="bs-example">
                        <form id="Input3" method="post">
									...
									<input class="btn btn-default btn-sm btn-xs" type="submit" value='6' />
