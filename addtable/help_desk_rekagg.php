...
<div></div>
    <div class="module_content">    
			...
			<?php
			$query = pg_query("select g_000001 as ktr,c_000001 as cif,a_000057 as nama,norek from m_01_002
			...
			while($fetch = pg_fetch_array($query))
			{
				...
				echo'<tr class="'.$class.'">
					...
					<td>'.$fetch['cif'].'</td>
					<td>'.$fetch['nama'].'</td>
					<td><span class= "xedit" id="'.$fetch['cif'].'">'.$fetch['norek'].'</span></td>
					</tr>';							
			}
...
jQuery(document).ready(function() {  
		...
		$(document).on('click','.editable-submit',function(){
			...
			$.ajax({
				url: "addtable/process_rekagg.php?id="+x+"&data="+y,
				type: 'GET',
				success: function(s){
					if(s == 'dana_blokir'){
					$(z).html(y);}
