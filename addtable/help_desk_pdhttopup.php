...
<script>
			$().ready(function(){
                $('#Input400').submit(function(e){
                    e.preventDefault();
					$('#content400').html('<img src="images/loading.gif"/>');
					$('#content400').slideDown('slow');
                    $.ajax({
                        'type': 'POST',
                        'url': 'addtable/help_desk_pdhttopup_update.php',
                        'data': $(this).serialize(),
						'cache': false,
						'async': true,
                        'success': function(html){
                            $('#content400').html(html);
                        }
                    });
                });
            });
...
where m_01_001.f_000021 = h_00_001.h_000001 and m_01_001.c_000001 = m_01_002.c_000001 
...
and m_01_001.c_000001 in (select norek from tagihan_kelompok_tbl where sts_diskon = '1')
...
<form id="Input400" method="post">
<input class="btn btn-default btn-sm btn-xs" type="submit" value='update'/>
<div id="content400"> klik untuk menampilkan data </div>
</form>
<?php } else {
