...
<script>
            $().ready(function(){
                $('#Input20').submit(function(e){
                    ...
                    $.ajax({
                        'type': 'POST',
                        'url': 'addtable/help_desk_kemarin_update.php',
                        'data': $(this).serialize(),
                        'cache': false,
                        'async': true,
...
$query = pg_query(" SELECT  kd_cab, kd_ao,  a.s_000000 as ao ,        
    ...
    where ktr in ('024','031','035','036','038','039','040','040','041','042','043','044') 
                    ...
                    mobile_cashier_trx.trx_date = h_00_001.h_000002)
                    group by kdstaf
                    order by kdstaf)
                    group by kd_ao,kd_cab, ao 
...
<br><form id="Input20" method="post">
<input class="btn btn-default btn-sm btn-xs" type="submit" value="update" onclick = " return confirm ('Anda Yakin Memproses Data?');") /><br>
<br>
<div id="content20"> Klik "Update" Untuk Proses Data </div></br>
</form></br>

</body>

<p style="text-align:center;"><font size="2">1.  Data Titipan Boleh Di Update Jika Sudah Melakukan Postingan </font></p>
