        ...
        <script>
			$().ready(function(){
                $('#prov').submit(function(e){
                    e.preventDefault();
					$('#rekagg').html('<img  src="images/loading.gif"/>');
					$('#rekagg').slideDown('slow');
                    $.ajax({
                        'type': 'POST',
                        'url': 'addtable/help_desk_rekagg.php',
                        'data': $(this).serialize(),
						'cache': false,
						'async': true,
...
<header><h3 class="tabs_involved">Edit Rekening Anggota</h3></header>
<div class="row">
